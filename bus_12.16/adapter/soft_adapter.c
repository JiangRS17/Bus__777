...
#include "../../bridge/bridge_error.h"
#include "../../bridge/transmission/trans_manager.c"
...
int AdapterSendRequest(SoftBusAdapter *adapter, PublishInfo *publishInfo, BridgeInfo *bridge)
{
    ...
    if (targetQueue->cnt < SOFTBUS_MAX_QUEUES)
    {
...
int32_t DestroySoftBusAdapter(int32_t adapterId)
{
    for (int i = 0; i < SOFTBUS_MAX_LISTS; i++)
    {
        if (globalAdapter[i] !=NULL && globalAdapter[i]->adapterId == adapterId)
        {
            ...
            // 清理桥节点引用
            for (int j = 0; j < SOFTBUS_MAX_LISTS;j++) {
