...
static int32_t *SoftBusMalloc(unsigned int size)
{
    if (size > MAX_MALLOC_SIZE){
        return NULL;
    }

    int32_t *memory = malloc(size);
    return memory;
}

static void SoftBusFree(void *pt)
{
    if (pt == NULL) {
        return;
    }
...
SoftBusQueues *CreateSoftBusList()
{
    ...
    // 分配内存给新的软总线
    SoftBusQueues *newSoftBus = (SoftBusQueues *)malloc(sizeof(SoftBusQueues));
    if (newSoftBus == NULL)
    {
